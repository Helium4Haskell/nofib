...
data Array i a = Array (i, i) [(i,a)]
array :: (i,i) -> [(i,a)] -> Array i a
array bounds values = Array bounds values

infix 5 !
(!) :: Eq i => Array i a -> i -> a
(Array _ arr) ! i = fromJust $ lookup i arr
...
four_partitions m =
  ...
  | i <- [0..(div m 4)],
    j <- [i..(div (m-i) 3)],
    k <- [(max j (ceiling ((intToFloat m)/(intToFloat 2)) - i - j))..(div (m-i-j) 2)],
    l <- [(m - (i+j+k))]]
